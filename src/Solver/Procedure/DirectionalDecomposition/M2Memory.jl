...
@inline function system_mass(M::M2Memory, grid)
    curr_state = current_state!(M)

    measure(grid) * vec(sum(abs2.(curr_state); dims = 1))
end

@inline function system_total_mass(M::M2Memory, PDE::PDEq,
...
@inline function system_total_mass(M::M2Memory, PDE::PDEq, grid,
